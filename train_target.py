...
from loss.im_loss import InformationMaximization
...
def train_epoch(epoch, model, target_dataset, optimizer, optimizer_centers, lr_scheduler, lr_scheduler_centers, pseudo_label, device, args):
    ## loss
    ...
    for i in range(0, num_iter):
        ...
        pred_t, fea_t = model.predict(data_target)
        t_label = pseudo_label[t_idx].long().to(device)
        centers = model.centers.micro_community.to(device)
        ...
        # optimization
        total_loss = args.alpha * loss_MSML + args.beta * loss_TSPE + args.gamma * loss_im + args.delta * loss_enc
        optimizer.zero_grad()
        ...
        lr_scheduler_centers.step(num_iter)
        avg_list.append(total_loss.item())
        if i % args.log_interval == 0:
            print(
                f"[Epoch:{epoch}/{args.nepoch}] => loss: {total_loss:.4f}\n")
    return avg_list

def train_model(args, model, train_loader, test_loader, folder_path, device, is_source=False):
    ...
    lr_scheduler_centers = StepwiseLR(optimizer_centers, total_epoch=args.nepoch, warm_up_iter=args.warm_up_iter,
                              init_lr=args.lr, gamma=0.0002, decay_rate=0.75, pretrained_flag=is_source)
    save_count = 0
    pseudo_label = None
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        # =====================  training start =================================
        if epoch == 1:
            print('start init centers...')
            _, _, _, pseudo_label = test(model, test_loader, device, is_updated=True)
        epoch_loss = train_epoch(epoch, model, train_loader_forever, optimizer, optimizer_centers, lr_scheduler, lr_scheduler_centers,
                                 pseudo_label, device, args)
        loss_history.append(epoch_loss)
        t_correct, correct_per_class, total_per_class, tmp_label = test(model, test_loader, device, is_updated=False)
        print(
...
def main():
    ...
    correct = 0
    stop = 0
    train_loader, test_loader = load_data(args.root_path, args.tar, args.batch_size, args.LDS_type, is_target=True)
    # ====================== load model =============================
    ...
    if backbone is not None:
        source_path = f"checkpoints/{get_dataset_name(args.src)}/source/{args.src}/model_best.pkl"
        print(f'load pretrained source model from {source_path}\n')
        model = torch.load(source_path).to(device)
        if args.bottleneck:
            model.centers = MicroCommunity(num_classes=args.num_classes, dimension=int(args.dimension),gar=args.gar).to(device)
        else:
            model.centers = MicroCommunity(num_classes=args.num_classes, dimension=model._out,gar=args.gar).to(device)
        folder_path = f"checkpoints/{get_dataset_name(args.tar)}/target/{args.src}_{args.tar}"
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        train_model(args, model, train_loader, test_loader, folder_path, device)
