...
class office31:
    ...
    def get_data(self):
        ...
        train_path = os.path.join('data/Office31/', '{}.txt'.format(self.name))
        test_path = os.path.join('data/Office31/', '{}.txt'.format(self.name))

        train_dataset = utils.ImageList(open(train_path).read().splitlines(), os.path.join(self.img_dir, 'images'))
        val_dataset = utils.ImageList(open(test_path).read().splitlines(), os.path.join(self.img_dir, 'images'))
        test_dataset = utils.ImageList(open(test_path).read().splitlines(), os.path.join(self.img_dir, 'images'))
        self.num_classes = 31
